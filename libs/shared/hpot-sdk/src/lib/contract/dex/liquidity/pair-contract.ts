...
import { DynamicFormatAmount } from '../../../utils';
...
export class PairContract implements BaseContract {
  ...
  get myLiquidityDisplay() {
    return this.token0LpBalance &&
      ...
      ? {
          reserve0: DynamicFormatAmount({
            amount: this.token0LpBalance.toString(),
            endWith: this.token0.displayName,
          }),
          reserve1: DynamicFormatAmount({
            amount: this.token1LpBalance.toString(),
            endWith: this.token1.displayName,
          }),
        }
      : {
          reserve0: '-',
  ...
  get liquidityDisplay() {
    return this.reserves?.reserve0 &&
      ...
      ? {
          reserve0: DynamicFormatAmount({
            amount: this.reserves?.reserve0.toString(),
            endWith: this.token0.displayName,
          }),
          reserve1: DynamicFormatAmount({
            amount: this.reserves?.reserve1.toString(),
            endWith: this.token1.displayName,
          }),
        }
      : {
