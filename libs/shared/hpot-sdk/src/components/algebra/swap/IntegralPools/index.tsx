...
const IntegralPools = () => {
  ...
  const { [SwapField.INPUT]: currencyA, [SwapField.OUTPUT]: currencyB } =
    currencies;

  const title =
    currencyA && currencyB && `${currencyA.symbol} / ${currencyB.symbol}`;

  const { isLoading: isPluginsLoading, dynamicFeePlugin } =
    usePoolPlugins(poolAddress);

  const [isOpen, setIsOpen] = useState<boolean>(false);

  return (
    <div className="relative flex flex-col w-full gap-2 py-2 pt-3 px-4 bg-card rounded-3xl border border-card-border">
      ...
      <div className="flex items-center justify-between">
        <Popover open={isOpen}>
          ...
          <PopoverContent
            ...
            onPointerDownOutside={() => setTimeout(() => setIsOpen(false), 0)}
          >
            <IntegralPoolsList
              poolAddress={poolAddress}
              onPoolSelect={() => setIsOpen(false)}
            />
          </PopoverContent>
        </Popover>
  ...
  ({
    ...
    return (
      <div className="flex flex-col gap-2">
        {pools?.pools
          .filter(
            (pool) => poolAddress?.toLowerCase() !== pool.id.toLowerCase()
          )
          .map((pool, idx) => (
            <IntegralPoolsListItem
              key={`integral-pool-item-${idx}`}
              pool={pool}
...
const IntegralPoolsListItem = memo(
  ({
    pool,
    onPoolSelect,
  }: {
    ...
    const { isLoading: isPluginsLoading, dynamicFeePlugin } = usePoolPlugins(
      pool.id as Address
    );
    ...
    return (
      <div
        className="flex items-center gap-4 min-h-[40px] text-white px-2 py-1 rounded-2xl whitespace-nowrap cursor-pointer hover:bg-card-hover duration-200"
        onClick={selectPool}
      >
        <div className="flex">
          <CurrencyLogo currency={currencyA} size={25} />
