...
import { useSwapChart } from "@/lib/algebra/hooks/swap/useSwapChart";
import {
...
const SwapChart = () => {
  ...
  const [chartType, setChartType] = useState<SwapChartViewType>(
    SwapChartView.LINE
  );
  const [chartSpan, setChartSpan] = useState<SwapChartSpanType>(
    SwapChartSpan.DAY
  );
  const [chartPair, setChartPair] = useState<SwapChartPairType>(
    SwapChartPair.AB
  );
  ...
  const [series, setSeries] = useState<
    LightWeightCharts.ISeriesApi<"Area" | "Candlestick"> | undefined
  >();
  ...
  useEffect(() => {
    ...
    if (
      (chartPair === SwapChartPair.AB || chartPair === SwapChartPair.BA) &&
      !poolAddress
    )
      return;
  ...
  const pairSelectorList = useMemo(() => {
    ...
    return Object.keys(SwapChartPair)
      .filter((v) => v !== chartPair)
      .map((pair: any) => ({
        pair,
  ...
  return (
    <div className="flex flex-col gap-6 w-full h-full relative">
      <MarketDepthChart
        ...
        close={() => setIsMarketDepthOpen(false)}
      />
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between">
        <Popover open={isPoolSwitcherOpen}>
          ...
          >
            ...
            <div>
              <ChevronDownIcon
                size={20}
                className={`duration-300 ${isPoolSwitcherOpen ? "rotate-180" : ""}`}
              />
            </div>
          </PopoverTrigger>
          ...
          <PopoverContent
            onPointerDownOutside={() =>
              setTimeout(() => setIsPoolSwitcherOpen(false), 0)
            }
            className="bg-card rounded-3xl border border-card-border"
          >
            <div className="flex flex-col gap-2 text-white">
              {pairSelectorList?.map((item) => (
                <div
        ...
        <div className="flex gap-4 w-fit p-2 bg-card border border-card-border rounded-3xl">
          <div className="flex gap-2">
            <Button
              variant={chartSpan === SwapChartSpan.DAY ? "iconActive" : "icon"}
              size={"icon"}
              onClick={() => setChartSpan(SwapChartSpan.DAY)}
            >
              1D
            </Button>
            <Button
              variant={chartSpan === SwapChartSpan.WEEK ? "iconActive" : "icon"}
              size={"icon"}
              onClick={() => setChartSpan(SwapChartSpan.WEEK)}
            >
              1W
            </Button>
            <Button
              variant={
                chartSpan === SwapChartSpan.MONTH ? "iconActive" : "icon"
              }
              size={"icon"}
              onClick={() => setChartSpan(SwapChartSpan.MONTH)}
            >
              1M
            </Button>
          </div>
          ...
          <div className="flex gap-2">
            <Button
              variant={chartType === SwapChartView.LINE ? "iconActive" : "icon"}
              size={"icon"}
              onClick={() => setChartType(SwapChartView.LINE)}
            >
            ...
            <HoverCard>
              <HoverCardTrigger>
                <Button
                  variant={
                    chartType === SwapChartView.CANDLES ? "iconActive" : "icon"
                  }
                  size={"icon"}
                  onClick={() => setChartType(SwapChartView.CANDLES)}
                  disabled
                >
          ...
          <HoverCard>
            <HoverCardTrigger>
              <Button
                variant={isMarketDepthOpen ? "iconActive" : "icon"}
                size={"icon"}
                onClick={() => setIsMarketDepthOpen((v) => !v)}
              >
