...
const MarketDepthChart = ({
  ...
  const [beforeCurrent, current, afterCurrent, maxLiquidity] = useMemo(() => {
    ...
    const slicedData = processedData
      .slice(middle - chunkLength, middle + chunkLength)
      .reverse();

    const currentIdx = slicedData.findIndex((v: any) => v.isCurrent);

    const maxLiquidity = slicedData.reduce(
      (acc: any, v: any) => (v.activeLiquidity > acc ? v.activeLiquidity : acc),
      0
    );

    return [
      slicedData.slice(0, currentIdx),
      slicedData[currentIdx],
      slicedData.slice(currentIdx + 1),
      maxLiquidity,
    ];
  }, [processedData]);

  const invertPrice =
    currencyA && currencyB && !currencyA.sortsBefore(currencyB);

  const tooltipY = useMemo(() => {
    if (hoveredIndex === NOT_SELECTED) return -9999;

    const id = document.getElementById(
      isAfterCurrent(hoveredIndex)
        ? `before-current-${hoveredIndex}`
        : `after-current-${-hoveredIndex}`
    );
  ...
  const tooltipData = useMemo(() => {
    if (hoveredIndex === NOT_SELECTED || !beforeCurrent || !afterCurrent)
      return;

    const summary = isAfterCurrent(hoveredIndex)
      ? beforeCurrent.slice(hoveredIndex)
      : afterCurrent.slice(0, -hoveredIndex + 1);
  ...
  const [highestTick, lowestTick] =
    beforeCurrent && afterCurrent
      ? [beforeCurrent[0].tick, afterCurrent[afterCurrent.length - 1].tick]
      : [];
  ...
  return (
    <div
      className={`h-[100vh] lg:h-full bg-card fixed border-l border-card-border right-0 top-0 left-0 bottom-0 lg:left-[unset] overflow-x-hidden lg:overflow-x-visible duration-200 z-[99] ${isOpen ? "w-[100vw] md:w-[380px]" : "w-[0px]"}`}
    >
      <div className="flex flex-col lg:flex-row w-full h-full">
        ...
        <div className="relative flex flex-col w-full h-full">
          ...
          <div
            className={`absolute lg:-left-[230px] z-[100]`}
          ...
          <div className="relative flex flex-col items-end w-full h-full overflow-y-auto pl-16 text-sm select-none">
            {isTickOutside === 1 && (
              <div
                className="flex items-center gap-2 py-1 w-full bg-yellow-600 mb-2 pl-4"
            ...
            {beforeCurrent
              ? beforeCurrent.map((v: any, idx: number) => (
                  <div
                    ...
                    onMouseOut={() => setHoveredIndex(NOT_SELECTED)}
                  >
                    <div className={`absolute flex justify-evenly w-full`}>
                      <div className="w-full text-left -ml-12 text-red-500 font-semibold">
                        {formatCurrency.format(
                          invertPrice ? v.price1 : v.price0
                        )}
                      </div>
                      <div className="w-full">
                        {formatCurrency.format(
                          invertPrice ? v.tvlToken1 : v.tvlToken0
                        )}
                      </div>
                      <div className="w-full text-right pr-4">
                        {formatCurrency.format(
                          invertPrice ? v.tvlToken0 : v.tvlToken1
                        )}
                      </div>
                    </div>
                    <div
                      ...
                      style={{
                        width: `${(v.activeLiquidity * 100) / maxLiquidity}%`,
                      }}
                    ></div>
                  </div>
                ))
            ...
            {current && (
              <div
                className="relative flex items-center justify-between text-lg py-2 px-4 border-y border-card-border my-2 bg-card-dark"
                style={{ width: "calc(100% + 4rem)" }}
              >
                <div>
                  {formatCurrency.format(
                    invertPrice ? current.price1 : current.price0
                  )}
                </div>
            ...
            {afterCurrent
              ? afterCurrent.map((v: any, idx: number) => (
                  ...
                  >
                    <div className="absolute flex justify-evenly w-full">
                      <div className="w-full text-left -ml-12 text-green-500 font-semibold">
                        {formatCurrency.format(
                          invertPrice ? v.price1 : v.price0
                        )}
                      </div>
            ...
            {isTickOutside === -1 && (
              <div
                className="flex items-center gap-2 py-1 w-full bg-yellow-600 mt-2 pl-4"
                style={{ width: "calc(100% + 4rem)" }}
              >
...
const MarketDepthChartTooltip = ({
  ...
  return (
    <div className="flex flex-col gap-2 p-4 rounded-2xl bg-card border border-card-border">
