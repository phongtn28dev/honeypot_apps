...
export function useSwapChart() {
  async function fetchPoolPriceData(
    address: string,
    span: "day" | "week" | "month"
  ) {
  ...
  async function fetchPriceData(
    ...
    const timestamp =
      span === "day" || span === "week" ? "periodStartUnix" : "date";
    ...
    try {
      ...
      while (!allFound) {
        ...
        if (!loading) {
          ...
          if (
            (priceData &&
              priceData[`${field}${fetchEntity}Datas`].length < 100) ||
            errors
          ) {
            allFound = true;
          }
      ...
      return {
        data: data.filter(
          (v, idx) => !periods.includes(v.periodStartUnix, idx + 1)
        ),
        error: false,
      };
    } catch (e) {
